<x-layout>
    ...
    <x-containers.main>
        ...
        <x-containers.content>
            <section class="grid grid-cols-6">
                ...
                <div class="col-span-4 grid gap-5 p-5">
                    <input type="text" name="fsic_id" class="text-field w-1/4" value="{{ old('fsic_id') ?? $certificate->fsic_id }}" placeholder="" form="fill-certificate" required>
                    <input type="date" name="filled_date" class="text-field w-1/4" value="{{ old('filled_date') ?? date($certificate->filled_date) }}" placeholder="" form="fill-certificate" required>
                    <input type="date" name="valid_until" class="text-field w-1/4" value="{{ old('valid_until') }}" placeholder="" form="fill-certificate" required>
                    <input type="hidden" name="occupancy_type" value="{{ old('occupancy_type') ?? $certificate->occupancy_type }}" placeholder="" form="fill-certificate" required>
                    <input type="hidden" name="issuance_type" value="{{ old('occupancy_type') ?? $certificate->issuance_type }}" placeholder="" form="fill-certificate" required>
                    ...
                    <div x-data="{ show: false }" class="w-1/3 relative">
                        <button @click="show = !show" @click.away="show = false" class="dropdown-trigger" type="button">
                            <span id="occupancy_label" class="font-barlow">{{ old('occupancy_label') ?? $certificate->occupancy_type }}</span>
                    ...
                    <div x-data="{ show: false }" class="w-1/3 relative">
                        <button @click="show = !show" @click.away="show = false" class="dropdown-trigger" type="button">
                            <span id="issuance_label" class="font-barlow">{{ old('issuance_type') ?? $certificate->issuance_type }}</span>
                    ...
                    <input type="text" name="others" class="text-field w-2/3" value="{{ old('others') ?? $certificate->others }}" placeholder="" form="fill-certificate">
                    <input type="text" name="description" class="text-field" value="{{ old('description') ?? $certificate->description }}" placeholder="ex. Lorem ipsum dolor sit amet." form="fill-certificate" required>
                </div>
            </section>
        </x-containers.content>
        ...
        <x-containers.content>
            <section class="grid grid-cols-6">
                ...
                <div class="col-span-4 grid gap-5 p-5">
                    <input type="text" name="establishment" class="text-field w-2/3" value="{{ old('establishment') ?? $certificate->applicant->establishment }}" placeholder="Establishment Name" form="fill-certificate" required>
                    <input type="text" name="firstname" class="text-field w-2/4" value="{{ old('firstname') ?? $certificate->applicant->firstname }}" placeholder="First Name" form="fill-certificate" required>
                    <input type="text" name="middlename" class="text-field w-2/4" value="{{ old('middlename') ?? $certificate->applicant->middlename }}" placeholder="Middle Name" form="fill-certificate" required>
                    <input type="text" name="lastname" class="text-field w-2/4" value="{{ old('lastname') ?? $certificate->applicant->lastname }}" placeholder="Last Name" form="fill-certificate" required>
                    <input type="text" name="address" class="text-field" value="{{ old('address') ?? $certificate->applicant->address }}" placeholder="Address" form="fill-certificate" required>
        ...
        <x-containers.content>
            <section class="grid grid-cols-6">
                ...
                <div class="col-span-4 grid gap-5 p-5">
                    <input type="number" name="amount" class="text-field w-1/4" value="{{ old('amount') ?? $certificate->payment->amount }}" placeholder="" form="fill-certificate" required>
                    <input type="text" name="or_number" class="text-field w-1/4" value="{{ old('or_number') ?? $certificate->payment->or_number }}" placeholder="" form="fill-certificate" required>
            ...
            <section class="flex p-5 space-x-4 font-barlowcondensed text-slate-700">
                <div class="w-1/2 space-y-3">
                    ...
                    <div x-data="{ show: false }" class="w-full relative">
                        <button @click="show = !show" @click.away="show = false" class="dropdown-trigger" type="button">
                            <span id="chief_label" class="font-barlowcondensed">{{ $certificate->chief }}</span>
                ...
                <div class="w-1/2 space-y-3">
                    ...
                    <div x-data="{ show: false }" class="w-full relative">
                        <button @click="show = !show" @click.away="show = false" class="dropdown-trigger" type="button">
                            <span id="marshal_label" class="font-barlowcondensed">{{ $certificate->marshal }}</span>
        ...
        <form action="{{ route('certificates.update', $certificate->id) }}" method="POST" id="fill-certificate">
            @csrf
            @method('PATCH')
            <input type="hidden" name="chief" value="{{ $certificate->chief }}" id="chief">
            <input type="hidden" name="marshal" value="{{ $certificate->marshal }}" id="marshal">
        </form>
    ...
    <div id="items"
         style="display: none"
         data-filled-date="{{ explode(' ', date($certificate->filled_date))[0] }}"
         data-valid-until="{{ explode(' ', date($certificate->filled_date))[0] }}"
         data-payment-date="{{ explode(' ', date($certificate->payment->date))[0] }}"
    >
    </div>
    ...
    <script>
        ...
        const items = document.getElementById('items').dataset;
        const details = Object.assign({}, items);

        document.querySelector('input[name="filled_date"]').value = details.filledDate;
        document.querySelector('input[name="valid_until"]').value = details.validUntil;
        document.querySelector('input[name="payment_date"]').value = details.paymentDate;
